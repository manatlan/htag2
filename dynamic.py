...
class DynamicHtagApps:
    ...
    async def __call__(self, scope, receive, send):
            ...
            if scope["type"] == "http":
                ...
                for entry in os.listdir(self.apps_dir):
                    if entry.endswith(".py") and entry != "__init__.py":
                        apps.append(entry.replace(".py", ""))
                    elif os.path.isdir(os.path.join(self.apps_dir, entry)):
                        if os.path.exists(os.path.join(self.apps_dir, entry, "__init__.py")):
                            apps.append(entry)
                apps.sort()
        ...
        if app_name not in self.apps_cache or self.apps_cache[app_name][0] < mtime:
            mod_name = f"apps.{app_name}"
            
            parent_dir = os.path.dirname(self.apps_dir)
            ...
            for name, obj in inspect.getmembers(mod, inspect.isclass):
                if issubclass(obj, Tag.App) and obj is not Tag.App:
                    app_class = obj
                    break
                    
            if not app_class:
                if scope["type"] == "http":
                    response = HTMLResponse("No Tag.App found in module", status_code=500)
                    await response(scope, receive, send)
                return
...
app = DynamicHtagApps("./apps")

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("dynamic:app", host="0.0.0.0", port=8001, reload=False)
