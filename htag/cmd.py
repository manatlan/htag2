...
def build(entrypoint: str):
    ...
    print(f"🚀 {C.BLUE}Building{C.END} '{C.BOLD}{app_name}{C.END}' from '{entry_path}'...")
    ...
    try:
        ...
        print(f"✅ {C.GREEN}{C.BOLD}Successfully built{C.END} '{C.BOLD}{app_name}{C.END}' in 'dist/' directory.")
...
class UvHelpFormatter(argparse.HelpFormatter):
    ...
    def _format_action_invocation(self, action):
        if not action.option_strings:
            return f"{C.GREEN}{C.BOLD}{super()._format_action_invocation(action)}{C.END}"
        else:
    ...
    def _format_action(self, action):
        if isinstance(action, argparse._SubParsersAction):
            ...
            for choice_action in action._choices_actions:
                ...
                parts.append(f"  {C.GREEN}{C.BOLD}{name:<8}{C.END} {C.DIM}{help_text}{C.END}")
            return "\n".join(parts) + "\n"
    ...
    def format_help(self):
        ...
        for line in lines:
            if re.search(r"(usage|Usage):", line, re.IGNORECASE):
                ...
                clean_line = clean_line.replace("{build}", f"{C.CYAN}<COMMAND>{C.END}{C.YELLOW}")
                new_lines.append(f"{C.YELLOW}{C.BOLD}Usage:{C.END} {C.YELLOW}{clean_line}{C.END}")
            else:
...
def main():
    ...
    parser.add_argument("-h", "--help", action="help", help="Display documentation for htagm")
    ...
    build_parser.add_argument("-h", "--help", action="help", help="Display help for the build command")
    build_parser.add_argument("path", help="Path to main script, or 'clear' to reset folders")
